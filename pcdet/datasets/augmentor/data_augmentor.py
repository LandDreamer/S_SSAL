...
from . import (
    augmentor_utils, database_sampler, database_sampler_naive,
    database_sampler_scene
)


class DataAugmentor(object):
    def __init__(self, root_path, augmentor_configs, class_names, logger=None, database_path=None):
        ...
        for cur_cfg in aug_config_list:
            ...
            self.data_aug_name.append(cur_cfg.NAME)
    ...
    def del_augmentation(self, cfg_names):
        for cur_cfg in cfg_names:
            for now_idx, now_aug in enumerate(self.data_augmentor_queue):
                if cur_cfg.NAME == self.data_aug_name[now_idx]:
                    del self.data_aug_name[now_idx]
                    del self.data_augmentor_queue[now_idx]
                    break

    def change_augmentation(self, augmentor_configs):
        aug_config_list = augmentor_configs if isinstance(augmentor_configs, list) \
            else augmentor_configs.AUG_CONFIG_LIST
        ...
        for cur_cfg in aug_config_list:
            ...
            for now_idx, now_aug in enumerate(self.data_augmentor_queue):
                if cur_cfg.NAME == self.data_aug_name[now_idx]:
                    cur_augmentor = getattr(self, cur_cfg.NAME)(config=cur_cfg)
                    self.data_augmentor_queue[now_idx] = cur_augmentor
                    break
    ...
    def gt_sampling_naive(self, config=None):
        db_sampler = database_sampler_naive.DataBaseSampler_Naive(
            root_path=self.root_path,
            sampler_cfg=config,
            class_names=self.class_names,
            logger=self.logger,
    ...
    def sampling_scene(self, config=None):
        db_sampler = database_sampler_scene.DataBaseSampler_Scene(
            root_path=self.root_path,
            sampler_cfg=config,
            class_names=self.class_names,
            logger=self.logger,
