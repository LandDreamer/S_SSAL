...
class DataBaseSampler(object):
    ...
    def filter_by_round(self, db_infos, sample_round):
        new_db_infos = {}
        for key, dinfos in db_infos.items():
            pre_len = len(dinfos)
            new_db_infos[key] = [
                info for info in dinfos
                if info['round'] in sample_round
            ]
            if self.logger is not None:
                self.logger.info('Database filter by round %s: %d => %d' % (key, pre_len, len(new_db_infos[key])))
        return new_db_infos
    ...
    def cer_class_indices(self, info, name):
        keep_indices = [i for i, x in enumerate(info) if name not in x]
        return keep_indices
