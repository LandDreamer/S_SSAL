...
class WaymoDataset_PseudoScene(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None, premode=None):
        ...
        self.sampleid_2_infoidx = {}

        self.repeat = self.dataset_cfg.get('REPEAT', 1)
        self.sample_out_dc = self.dataset_cfg.get('SAMPLE_OUT_DC', False)
        ...
        if self.logger is not None:
            self.logger.info('Total samples for Waymo dataset: %d' % (len(self.infos)))
        self.ssl_mode = self.dataset_cfg.get('SSL_Mode', True)
        self.set_ssl_mode(self.ssl_mode)
    ...
    def get_cer_info(self, sample_idx):
        self.get_sampleid_2_infoidx()
        info_idx = self.sampleid_2_infoidx[sample_idx]
        return self.infos[info_idx]

    def get_sampleid_2_infoidx(self):
        if len(self.sampleid_2_infoidx) != len(self.infos):
            self.sampleid_2_infoidx = {info['frame_id']: idx for idx, info in enumerate(self.infos)}
        return self.sampleid_2_infoidx
    ...
    def include_waymo_data(self, mode):
        ...
        self.frame_ids = [info['frame_id'] for info in self.infos]
    ...
    def load_data_to_shared_memory(self):
        ...
        for info in cur_infos:
            ...
            sequence_name = pc_info['lidar_sequence'][0][0]
    ...
    def __len__(self):
        ...
        if self.training:
            return int(len(self.infos) * self.repeat)
        else:
    ...
    def __getitem__(self, index):
        ...
        if self.training and self.ssl_mode:
            use_ssl = np.random.randint(-1,self.pseudo_num)
            if use_ssl <= 0:
                # data_dict_labeled = self.get_item_single(info)
                data_dict = data_dict_labeled
            else:
                index_unlabeled = np.random.choice(len(self.unlabeled_frame_ids), 1)[0]
    ...
    def get_item_single(self, info, is_ssl=False):
        ...
        if 'annos' in info:
            ...
            if is_ssl:
                annos = common_utils.keep_info_with_name(annos, name='unknown')
