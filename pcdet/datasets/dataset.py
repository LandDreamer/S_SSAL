...
class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None, premode=None):
        super().__init__()
        ...
        self.init_database_path = self.dataset_cfg.get('INIT_DATABASE_PATH', None)
        self.database_path = self.dataset_cfg.get('DATABASE_PATH', None)
        if self.init_database_path is not None:
            self.init_database_path = Path(self.init_database_path)
            # 
        if self.database_path is not None and os.path.exists(self.database_path):
            self.database_path = Path(self.database_path)
            if self.logger is not None:
                self.logger.info('Use new construct database')
        else:
            self.database_path = self.init_database_path
            if self.logger is not None:
                self.logger.info('Use init construct database')
        self.logger = logger
    ...
    def prepare_data(self, data_dict):
        ...
        if data_dict.get('gt_boxes', None) is not None:
            ...
            if not self.training and 'num_points_in_gt' in data_dict:
    ...
    def collate_batch(batch_list, _unused=False):
        ...
        for key, val in data_dict.items():
            try:
                ...
                elif key in ['gt_names', 'sequence_name', 'num_points_in_gt', 'occluded', 'truncated']:
                    if key not in ret:
                        ret[key] = []
                    ret[key].append(val)
                else:
