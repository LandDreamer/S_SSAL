...
from pcdet.datasets import build_dataloader, build_ssl_dataloader_naive, build_active_dataloader
...
def main():
    ...
    train_set, train_loader, train_sampler = build_ssl_dataloader_naive(
        ...
        save_pre_sample=cfg.ALSSL_TRAIN.get('SAVE_PRE_SAMPLE', False), 
        use_pre_sample=cfg.ALSSL_TRAIN.get('USE_PRE_SAMPLE', True), 
        save_sample_path_list=cfg.ALSSL_TRAIN.get('SAVE_SAMPLE_PATH', [None]) 
    )
    if cfg.ALSSL_TRAIN.get('DATABASE_RECONSTRUCT_INTERVAL', -1) > 0:
        labeled_set, unlabeled_set, labeled_loader, unlabeled_loader, \
            ...
            save_sample_path=cfg.ALSSL_TRAIN.get('SAVE_SAMPLE_PATH', [None]) , shuffle=False
        )
        unlabeled_datalodar = unlabeled_loader
    else:
        unlabeled_datalodar = None
    ...
    train_model(
        ...
        cfg=cfg,
        stop_train_interval=cfg.ALSSL_TRAIN.get('SCENE_SAMPLE_INTERVAL', None),
        database_save_dir=cfg.ALSSL_TRAIN.get('DATABASE_SAVE_DIR', None),
        database_recontruct_interval=cfg.ALSSL_TRAIN.get('DATABASE_RECONSTRUCT_INTERVAL', -1),
        unlabeled_datalodar = unlabeled_datalodar,
    )
    ...
    repeat_eval_ckpt(
        ...
        dist_test=dist_train, eval_max_epoch=True
    )
